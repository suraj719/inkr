...
export function isWithinElement(x, y, element) {
  ...
  switch (tool) {
    ...
    case "circle":
      const circleWidth = x2 - x1 + strokeWidth;
      const circleHeight = y2 - y1 + strokeWidth;
      x1 -= strokeWidth / 2;
      y1 -= strokeWidth / 2;

      const centreX = x1 + circleWidth / 2;
      const centreY = y1 + circleHeight / 2;
    ...
    case "text":
      ...
      return x >= x1 && x <= x1 + textWidth && y >= y1 && y <= y1 + textHeight;
  }
}
...
export function saveElements(elements, format = 'inkr') {
  if (format === 'png') {
    saveCanvasAsPNG();
    return;
  }
  ...
  function uploadJSON(event) {
    ...
    reader.onload = (e) => {
      try {
        ...
        console.error("Error loading INKR file:", error);
      }
    };
